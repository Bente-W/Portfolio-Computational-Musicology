...
MeanFreddie <- freddie %>%
  ...
  summarise(mean_pop = mean(track.popularity), 
            mean_danceability = mean(danceability),
            mean_tempo = mean(tempo)) %>%
  mutate(category = "Freddie Mercury")
...
MeanRoger <- roger %>%
  ...
  summarise(mean_pop = mean(track.popularity), 
            mean_danceability = mean(danceability),
            mean_tempo = mean(tempo)) %>%
  mutate(category = "Roger Taylor")

Total <- rbind(MeanFreddie, MeanQueen, MeanRoger)

Test <- Tempo %>%
  plot_ly(x = ~tempo_section_mean, y = ~tempo_section_sd, size = ~danceability, color = ~track.album.release_date,
    hoverinfo = "text", text = paste("Track: ", Tempo$track, "<br>",
                                   "Artist: ", Tempo$category, "<br>",
                                   "Release Date: ", Tempo$track.album.release_date, "<br>",
                                   "Tempo: ", Tempo$tempo_section_mean)) %>%
  add_text(x = 150, y = 40, text = ~category, id = ~1, frame = ~category,
           textfont = list(size = 20, color = toRGB("gray80"))) %>%
  add_markers(frame = ~category) %>%
  layout(title = 'Tempo for Each Artist', xaxis = list(title = "Mean Tempo (bpm)"), yaxis = list(title = 'SD Tempo'), showlegend = FALSE) %>%
  animation_opts(frame = 2000, 
                 transition = 1500,
                 easing = "linear") %>%
...
### Other matrices and the conclusion. {data-commentary-width=350}
...
### The song that was written by Roger Taylor has a clearer tempo in the tempogram. {data-commentary-width=450}

```{r}
CarTempogram <- readRDS("CarTempogram")
ggplotly(CarTempogram)
```
***

**Explanation**  
[This](https://open.spotify.com/track/253ewjP2R1LjKqA70mGdnk?si=eb97228f37284740) clearly structured song (as could be seen in the self-similarity matrices) written by Roger Taylor has faintly more of a coherent tempo. The uptempo rock song has a driving rhythm and features Taylor's vocals and a prominent guitar riff from Brian May. The lyrics are about Taylor's love for his car and as I said before, the song also includes some car sounds, but that did not change the accuracy of the tempogram.
...
### The song *Barcelona* by Freddie Mercury and opera singer Montserrat Caballé is all over the place tempo wise. {data-commentary-width=450}
...
***
**Explanation**  
The song [Barcelona](https://open.spotify.com/track/3BdId1EhcMX7yAzwzQJLAD?si=7cee17fa0b0943e7) is very "un-rock ’n’ roll" as Freddie Mercury stated himself. It "really required a lot of discipline" and "[the] ideas [...] underwent significant changes along the road to perfection." ([Freddie Mercury's official site](http://www.freddiemercury.com/en/archive#Barcelona_original)). In other words, Freddie Mercury put a lot of time and effort in the songs on the album *Barcelona* to create something really unique.  


### All different tempi for each artist. {data-commentary-width=450}
...
```{r}
lovekills <-
  get_tidy_audio_analysis("1cApo5IeHOea0dD3Cs3QMB") |>
  select(segments) |>
  unnest(segments)
...
### Electronic disco song *Let's Turn it On* by Freddie Mercury. {data-commentary-width=450}
...
```{r}
Comparison3 <- Comparison2 %>% select(danceability, energy, key, loudness, mode, speechiness, acousticness, instrumentalness, liveness, valence, tempo, time_signature, track.duration_ms, popularity)
...
for (i in 1:15) {
  ...
  artists.out <- kmeans(Comparison3, centers = i, nstart = 20, iter.max = 50)
  # Save the within cluster sum of squares
  wss[i] <- artists.out$tot.withinss
}
...
artists.out <- kmeans(Comparison3, centers = 3, nstart = 20, iter.max = 50)

# View the resulting model
# artists.out

# Plot of Defense vs. Speed by cluster membership
plot(Comparison3[, c("danceability", "energy")],
     col = artists.out$cluster,
     main = paste("k-means clustering of all songs with", k, "clusters"),
     xlab = "Danceability", ylab = "Energy")
ggplot(Comparison2, aes(danceability, energy, color = category)) +
  geom_point(shape = 21)
```
